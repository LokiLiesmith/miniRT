...
/*   Updated: 2025/12/05 16:19:29 by mrazem           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				mouse_drag(mouse_key_t button, action_t action,
						modifier_key_t mods, void *param);
void				mouse_scroll(double xdelta, double ydelta, void *param);

// object_movement.c
void				move_sphere(t_rt *rt, t_sphere *sph,
						double speed, t_mov_dir dir);
void				move_plane(t_rt *rt, t_plane *pl,
						double speed, t_mov_dir dir);
void				move_cylinder(t_rt *rt, t_cylinder *cy, double speed,
						t_mov_dir dir);
void				move_object(t_rt *rt, t_object *selected, double speed,
						t_mov_dir dir);
void				move_selection(mlx_key_data_t keydata, t_rt *rt);

// object_rotation.c
void				rotate_cylinder(t_rt *rt, t_cylinder *cy, t_rot_dir rot_dir,
						double angle);
void				rotate_plane(t_rt *rt, t_plane *pl, t_rot_dir rot_dir,
						double angle);
void				rotate_object(t_rt *rt, t_object *selected,
						t_rot_dir rot_dir);
void				rotate_selection(mlx_key_data_t keydata, t_rt *rt);

// object_scaling.c
void				scale_cylinder(mlx_key_data_t keydata, t_cylinder *cy,
						t_scale par);
void				scale_sphere(mlx_key_data_t keydata, t_sphere *sp,
						t_scale par);
...
//REPLACED BY THE vec_rot_around_axis
...
int					parse_cy_cont(int i, double xyz[3],
						char **strs, t_cylinder *cy);
...
// colors.c
uint32_t			calculate_color(t_rt *rt, t_hit hit, unsigned int x,
						unsigned int y);
...
// shadows_2.c
bool				is_backfacing(t_vec3 normal, t_vec3 point,
						t_vec3 light_pos);

// shadows.c
double				calc_soft_shadow(t_rt *rt, t_hit hit, unsigned int x,
						unsigned int y);
// view_and_ray.c
